<?php
// This script display personal post (personal tag page)
include 'connect.php';
$tag_name = $_POST['tag_name'];
$more_posts = $_POST['more_posts'];
$output = '';
$fetch_posts = $conn->query("SELECT * FROM posts WHERE tag_name = '$tag_name'ORDER BY id DESC LIMIT $more_posts") or die(mysqli_error($conn));
if (mysqli_num_rows($fetch_posts) > 0) {
	while ($post_row = mysqli_fetch_assoc($fetch_posts)) {
		...
		$fetch_admin_id = $conn->query("SELECT * FROM admin WHERE id = '$admin_id'") or die(mysqli_error($conn));
...
<script type="text/javascript">
	//This script hides the view more button after all posts are displayed
